...
<div class="profile-edit">
    ...
    <div class="profile-edit__inner">
        <form action="/mypage/profile" class="profile-edit__form" method="POST">
            @csrf
            <div class="profile-edit__image-area">
                <img src="{{ $profile->image ? asset('storage/' . $profile->image) : asset('images/default-user.png') }}" class="profile-edit__img" alt="プロフィール画像"/>
                <label class="profile-edit__image-label" for="image">画像を選択する</label>
                <input class="profile-edit__image-input" type="file" name="image" id="image">
            </div>
